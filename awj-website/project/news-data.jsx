/* AWJ News — articles + topical placeholder cover SVG generators.
   Each cover is a composed SVG illustration that expresses the topic without
   resorting to icons-on-gradient slop. Used in both Landing News section and News.html.
*/
...
const NEWS = [
  {
    ...
    category: "Healthcare",
    title: "AWJ Innovation and Sultan Qaboos Hospital Launch Healthcare Innovation Studio",
    date: "2024-11-03",
...
/* Category visual system — each category gets a base color and SVG cover.
   Covers are designed as small editorial illustrations (not icons on gradient).
*/
...
/* SVG cover by category — returns a JSX element. 16:10 design canvas. */
const NewsCover = ({ category, compact = false }) => {
  ...
  const Bg = (
    <>
      <defs>
        <linearGradient id={`bg-${category.replace(/\s/g, '')}`} x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor={s.a} />
          <stop offset="100%" stopColor={s.b} />
        </linearGradient>
        <pattern id={`grain-${category.replace(/\s/g, '')}`} width="3" height="3" patternUnits="userSpaceOnUse">
          <circle cx="1" cy="1" r="0.4" fill="#fff" opacity="0.05" />
        </pattern>
      </defs>
      <rect width={W} height={H} fill={`url(#bg-${category.replace(/\s/g, '')})`} />
      <rect width={W} height={H} fill={`url(#grain-${category.replace(/\s/g, '')})`} />
    </>
  );
    ...
    switch (category) {
      case "Healthcare":
        return (
          <g>
            {/* layered ECG line */}
            <path d="M0 600 L300 600 L360 380 L420 820 L500 600 L900 600 L960 460 L1020 720 L1100 600 L1600 600"
              stroke={s.accent} strokeWidth="6" fill="none" opacity="0.85" />
            <path d="M0 700 L400 700 L460 540 L520 860 L600 700 L1600 700"
              stroke="#fff" strokeWidth="2" fill="none" opacity="0.35" />
            {/* cross motif */}
      ...
      case "Digital Transformation":
        return (
          <g>
            ...
            {/* nodes */}
            {[[200,200],[1400,300],[1300,800],[300,750],[1100,150]].map(([x,y],i) => (
                ...
                [0,1,2,3].map(col => {
                  ...
                  return (
                    <g key={`${row}-${col}`} transform={`translate(${x}, ${y})`}>
                      <path d={`M0 0 L120 0 L180 -40 L60 -40 Z`} fill={col%2 ? s.accent : "#fff"} opacity={0.85 - row*0.15} />
                      <path d={`M0 0 L0 120 L120 120 L120 0 Z`} fill={col%2 ? "#fff" : s.accent} opacity={0.65 - row*0.15} />
                      <path d={`M120 0 L180 -40 L180 80 L120 120 Z`} fill="#000" opacity="0.25" />
                    </g>
                  );
                })
              )}
          ...
          <g>
            ...
            <g>
              ...
              {Array.from({length: 6}).map((_, i) => {
                ...
                const cx = 800;
                const cy = 1000 - t * 600;
                return <rect key={i} x={cx - w/2} y={cy - (60 - t*48)} width={w} height={40 - t*32} fill={s.accent} opacity={0.85 - t*0.4} />
              })}
            </g>
            {/* horizon arcs */}
      ...
      case "Training":
        return (
          <g>
            {/* layered chevrons (progression) */}
            {[0,1,2,3,4].map(i => (
              <path key={i}
                d={`M${100 + i*200} 200 L${300 + i*200} 500 L${100 + i*200} 800`}
                fill="none" stroke={s.accent} strokeWidth="6"
                opacity={0.3 + i*0.15} />
            ))}
            {/* people glyphs */}
            <g fill={s.accent} opacity="0.85">
        ...
        return (
          <g>
            ...
            <g>
              ...
              {[[300,300],[1300,280],[1200,780],[400,800],[600,200],[1000,900]].map(([x,y],i)=>(
                <g key={i}>
      ...
      case "Urban Development":
        return (
          <g>
            {/* axonometric city blocks */}
            <g transform="translate(400, 200)">
              {[
                [0,0,180,1],[200,0,140,0.85],[360,0,220,0.7],
                [0,200,260,0.9],[280,200,180,1],[480,200,140,0.8],
                [0,420,180,0.75],[200,420,260,0.95]
              ].map(([x,y,h,o],i) => (
                <g key={i} transform={`translate(${x}, ${y})`}>
                  {/* top */}
                  <path d={`M0 ${-h} L120 ${-h-40} L240 ${-h} L120 ${-h+40} Z`} fill={s.accent} opacity={o} />
                  {/* left face */}
                  <path d={`M0 ${-h} L120 ${-h+40} L120 40 L0 0 Z`} fill="#fff" opacity={o*0.7} />
                  {/* right face */}
                  <path d={`M240 ${-h} L120 ${-h+40} L120 40 L240 0 Z`} fill="#000" opacity={0.35} />
                  {/* windows */}
                  {Array.from({length: Math.floor(h/40)}).map((_, w) => (
                    <rect key={w} x="20" y={-h + 30 + w*40} width="20" height="14" fill="#fff" opacity={o*0.5} />
                  ))}
                </g>
              ))}
            </g>
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid slice" className="news-cover-svg">
      {Bg}
      {renderArt()}
    </svg>
  );
...
window.NEWS = NEWS;
window.CATEGORY_STYLES = CATEGORY_STYLES;
window.NewsCover = NewsCover;
