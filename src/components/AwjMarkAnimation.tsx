import type { CSSProperties } from 'react';

/**
 * The AWJ mark, animated: each path traces its outline (stroke wipe) and then
 * fills in. Ported from the provided "AWJ Logo Animation" prototype — same
 * pieces, timing and easing. One-shot on mount.
 */
type Piece = { d: string; fill: string; delay: number };

const PIECES: Piece[] = [
  {
    d: 'M172.96,145.01H0L86.51,0l18.82,31.56c4.94,8.56,5.28,15.77,5.11,22.19-.3-.06-.6-.11-.89-.17-2.52-.46-4.89-.91-6.95-1.57.04-5.16-.53-10.34-4.07-16.5l-12.02-20.14L13.86,137.14h145.34c-4.61-7.82-12.55-21.21-15.92-26.55-4.65-7.38-11.25-12.01-17.73-16.39-.33-.22-.65-.44-.98-.66-2.07-1.39-4.16-2.8-6.19-4.3-4.59-3.38-8.87-7.21-11.92-12.38-3.54-6.01-4.12-11.48-4.09-16.74,1.92.5,3.87.86,5.75,1.2.72.13,1.44.27,2.14.4.1,3.75.73,7.33,2.98,11.14,1.94,3.3,4.64,5.99,7.74,8.46,2.06,1.65,4.31,3.2,6.61,4.76.46.31.91.61,1.37.92,7.13,4.79,15.22,10.23,20.99,19.39,4.93,7.82,18.94,31.68,19.54,32.69l3.48,5.93Z',
    fill: '#8d6fdb',
    delay: 0,
  },
  {
    d: 'M139.92,121.86c-.1-.17-9.88-16.67-13.07-22.17-1.02-1.76-1.38-3.59-1.29-5.49-.33-.22-.65-.44-.98-.66-2.07-1.39-4.16-2.8-6.19-4.3-1.08,4.57-1.19,9.5,1.66,14.41,1.97,3.39,6.42,10.93,9.59,16.29H43.38l43.2-73.28c1.05,1.71,2.17,3.55,3.12,5.12,3.01,4.96,7.75,7.08,12.67,8.34,1.92.5,3.87.86,5.75,1.2.72.13,1.44.27,2.14.4,5.12,1,9.45,2.2,11.93,5.95,2.35,3.57,1.09,7.53-1.2,13.65,2.06,1.65,4.31,3.2,6.61,4.76.21-.57.43-1.13.65-1.72,2.46-6.5,5.23-13.87.52-21.03-4.46-6.75-11.72-8.36-18.32-9.6-.3-.06-.6-.11-.89-.17-2.52-.46-4.89-.91-6.95-1.57-2.68-.87-4.84-2.13-6.16-4.31-2.63-4.34-6.57-10.74-6.57-10.74l-3.42-5.57L29.61,127.8h113.83l-3.52-5.94Z',
    fill: '#5f3fc0',
    delay: 0.7,
  },
  {
    d: 'M109.55,53.58c-2.52-.46-4.89-.91-6.95-1.57-.02,1.31-.07,2.62-.12,3.94-.05,1.38-.1,2.76-.11,4.16,1.92.5,3.87.86,5.75,1.2.72.13,1.44.27,2.14.4-.05-1.77.02-3.58.09-5.47.03-.82.06-1.65.09-2.5-.3-.06-.6-.11-.89-.17Z',
    fill: '#5f3fc0',
    delay: 0.7,
  },
  {
    d: 'M112.28,110.65h-51.62l25.81-45.46,25.81,45.46ZM74.18,102.78h24.58l-12.29-21.64-12.29,21.64Z',
    fill: '#a98ee8',
    delay: 1.4,
  },
];

const SPEED = 1;
const DRAW = 2.2 / SPEED; // outline draw time per piece
const FILL = 0.9 / SPEED; // fill fade time

export const AwjMarkAnimation = ({ className }: { className?: string }) => (
  <svg
    className={`awj-mark-anim${className ? ' ' + className : ''}`}
    viewBox="0 0 172.96 145.01"
    style={{ width: '100%', height: 'auto', overflow: 'visible', display: 'block' }}
    aria-hidden="true"
  >
    {PIECES.map((p, i) => (
      <path
        key={i}
        d={p.d}
        fill={p.fill}
        fillOpacity={0}
        pathLength={1}
        stroke={p.fill}
        strokeWidth={0.9}
        style={
          {
            strokeDasharray: 1,
            strokeDashoffset: 1,
            animation:
              `awjOutline ${DRAW}s cubic-bezier(.45,.1,.35,.95) ${p.delay / SPEED}s forwards, ` +
              `awjFill ${FILL}s ease ${p.delay / SPEED + DRAW * 0.85}s forwards`,
          } as CSSProperties
        }
      />
    ))}
  </svg>
);
